...
class IngestionActivityTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_embed_and_index_maps_chunks_then_rebuilds_lexical_index(self):
        ...
        self.assertEqual(
            indexed,
            [
                Chunk(make_chunk_id("policy.pdf", 1, 0), "first", "policy.pdf", 1, 0),
                Chunk(make_chunk_id("policy.pdf", 3, 1), "second", "policy.pdf", 3, 1),
            ],
        )
    ...
    async def test_embed_and_index_handles_empty_document_consistently(self):
        with patch(
            ...
            result = await activities.embed_and_index([], "empty.txt")
...
class IngestionWorkflowTest(unittest.IsolatedAsyncioTestCase):
    async def test_native_document_runs_persist_then_index_pipeline(self):
        ...
        with patch(
            ...
            result = await IngestDocumentWorkflow().run("/tmp/policy.txt", "policy.txt")
        ...
        self.assertEqual(execute_activity.await_args_list[3].kwargs["args"], [chunks, "policy.txt"])
        self.assertEqual(execute_activity.await_args_list[4].kwargs["args"], [chunks, "policy.txt"])
        self.assertEqual(
            result,
            {"filename": "policy.txt", "num_chunks": 1, "stored": True, "indexed": True},
        )
    ...
    async def test_scanned_document_selects_textract_before_indexing(self):
        ...
        with patch(
            ...
            result = await IngestDocumentWorkflow().run("/tmp/scan.pdf", "scan.pdf")
...
class BackfillIndexTest(unittest.TestCase):
    def test_load_chunks_maps_postgres_rows_and_closes_connection(self):
        ...
        self.assertEqual(
            chunks,
            [
                Chunk(make_chunk_id("a.pdf", 1, 0), "first", "a.pdf", 1, 0),
                Chunk(make_chunk_id("a.pdf", 2, 1), "second", "a.pdf", 2, 1),
            ],
        )
    ...
    def test_main_indexes_loaded_chunks_and_rebuilds_bm25(self):
        chunks = [Chunk("one", "text", "a.pdf", 1, 0)]
        with patch.object(
